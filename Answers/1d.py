...
for r in range(nComb):
   ...
   allCombResult.append([r, modelTerm, len(modelTerm), modelLLK, modelDF, AIC, BIC, n_sample,accuracy])
...
allCombResult = pandas.DataFrame(allCombResult, columns = ['Step', 'Model Term', 'Number of Terms', \
                ...
                'Bayesian Information Criterion', 'Sample Size','Accuracy of TestData'])
elapsedTime = endTime - startTime
max_testaccuracy = allCombResult.loc[allCombResult['Accuracy of TestData'].idxmax()]
print("highest Accuracy value on the Testing partition",max_testaccuracy['Accuracy of TestData'])
print("model producing the highest Accuracy value on the Testing partition",max_testaccuracy['Model Term'])